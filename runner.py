...
class Runner(object):
    def __init__(self, meta_agent_id):
        ...
        self.local_policy_network = PolicyNet(INPUT_DIM, EMBEDDING_DIM)
        self.local_critic_network = CriticNet(INPUT_DIM, EMBEDDING_DIM)
        self.local_policy_network.to(self.device)
        self.local_critic_network.to(self.device)

    def get_weights(self):
        return self.local_policy_network.state_dict()

    def set_policy_net_weights(self, weights):
        self.local_policy_network.load_state_dict(weights)

    def set_q_net_weights(self, weights1):
        self.local_critic_network.load_state_dict(weights1)

    def do_job(self, episode_number):
        save_img = True if episode_number % SAVE_IMG_GAP == 0 else False
        # save_img = True
        worker = Worker(self.meta_agent_id, self.local_policy_network, self.local_critic_network, episode_number,
                        device=self.device, greedy=False, save_image=save_img)
